...
var sweetTitles = { 
	...
	tipShow : function() {		
		...
		if ( anch.nodeName.toLowerCase() == 'a') {		
			var host = location.hostname.toString(); /*Anch.href to path start*/
			var pattern = new RegExp("({entryrelurl}?)"+host,"gi");
			var fullPath = anch.href.toString();
			var path = fullPath.replace(pattern,''); /*Anch.href to path end*/		
		if(anch.href.toString().indexOf("javascript:")==-1){
			addy ="Link to: <span>"+ (path.length > 80 ? path.toString().substring(0,80)+"...</span>" : path);
			//addy ="链接到:<span>"+ (anch.href.length > 45 ? anch.href.toString().substring(0,45)+"...</span>" : anch.href);
		}
